// V2: different to the V1 this deletes all but the last backup of each day of the current month
...
const testData = [
    ...
    '2023-07-16T01:00:00.zip',
    '2023-07-16T02:00:00.zip',
    '2023-07-16T03:00:00.zip',


    '2023-07-17T01:00:00.zip',
    '2023-07-17T10:00:00.zip',
    '2023-07-17T13:00:00.zip',
...
const filter30firstDays = () => {
    ...
    const groupedArray = Object.values(onlyCurrentMonth.reduce((x, date) => {
        const day = date.split('-')[2].split("T")[0];
        const [year, month] = date.split('-');
        const keys = `${year}_${month}_${day}`;
        x[keys] = x[keys] || { days: [] };

        x[keys].days.push(date)
    ...
    const filterLastOneOfDay = groupedArray.flatMap((x) => {
        return x.days.filter((d, i) => {
            return i != x.days.length - 1
        })
    }

    )
    return filterLastOneOfDay
...
deleteBackup(filter30firstDays(), "Deleting all but the last one of each day of curent month")
